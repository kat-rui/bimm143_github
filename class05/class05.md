...
> **Note to Kate** We never run an install.packages in a code chunk
> otherwise we will re-install needlessly every time we render our
> document. Do it in the console instead.
...
1.  The **data** i.e. stuff plot as data.frame
...
ggplot(genes) +
  ...
  labs(x="Control(no drug)",
       y="Drug",
       title="Expression chages with GLP-1") +
  theme_minimal()
```
...
``` r
input<-filter(gapminder, year ==1977| year== 2007)
table(input$year)
```
